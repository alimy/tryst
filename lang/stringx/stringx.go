...
import (
	...
	"github.com/alimy/tryst/lang/dirtmake"
	"github.com/alimy/tryst/lang/fastrand"
)
...
func Reverse(s string) (string, error) {
	...
	size := len(s)
	src := unsafe.Slice(unsafe.StringData(s), size)
	dst := dirtmake.Bytes(size, size)
	srcIndex := size
	dstIndex := 0
	for srcIndex > 0 {
		r, n := utf8.DecodeLastRune(src[:srcIndex])
		if r == utf8.RuneError {
			return unsafe.String(unsafe.SliceData(dst), len(dst)), ErrDecodeRune
		}
